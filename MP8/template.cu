...
__global__ void spmvJDSKernel(float *out, int *matColStart, int *matCols,
  ...
  if(row < num_rows){
    float dot = 0;
    unsigned int sec = 0;
    // while(matCols[sec + 1] - matCols[sec] > row)
...
static void spmvJDS(float *out, int *matColStart, int *matCols,
  ...
  wbTime_start(Compute, "CUDA computation: invoking spmv kernel...");
  float TILE_WIDTH = 1024;
  spmvJDSKernel<<<ceil(1.0*dim/TILE_WIDTH) ,TILE_WIDTH>>>(out, matColStart, matCols, matRowPerm, matRows, matData, vec, dim);
  wbTime_stop(Compute, "CUDA computation done");
